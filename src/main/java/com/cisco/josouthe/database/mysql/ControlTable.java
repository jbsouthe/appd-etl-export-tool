...
public class ControlTable extends MySQLTable implements com.cisco.josouthe.database.ControlTable {
    ...
    public synchronized int setLastRunTimestamp(ControlEntry controlEntry) {
        try (Connection conn = database.getConnection(); Statement statement = conn.createStatement();){
            StringBuilder update = new StringBuilder(String.format(" merge %s using dual on ( lower(controller) like lower('%s') and lower(application) like lower('%s') and lower(dataType) like lower('%s') )", this.name, controlEntry.controller, controlEntry.application, controlEntry.type ));
