...
public class MetricTable extends MySQLTable implements com.cisco.josouthe.database.MetricTable {
    ...
    public MetricTable( String tableName, Database database ) {
        ...
        for( String columnName : new String[] { "metricid","startTimeInMillis", "occurrences", "currentValue", "min", "max", "count", "sum", "value"})
            columns.put(columnName.toLowerCase(), new ColumnFeatures(columnName, "bigint", 20, false));
        columns.put("standardDeviation", new ColumnFeatures("standardDeviation", "decimal", 10, false));
        columns.put("startTimestamp", new ColumnFeatures("startTimestamp", "datetime", -1, false));
        this.initTable();
    }
