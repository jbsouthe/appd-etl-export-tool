...
import com.cisco.josouthe.database.oracle.*;
...
public class MicrosoftDatabase extends Database {
    ...
    public MicrosoftDatabase(Configuration configuration, String connectionString, String user, String password, String metricTable, String controlTable, String eventTable, String baselineTable, Long firstRunHistoricNumberOfHours) throws InvalidConfigurationException {
    ...
    public Connection getConnection() throws SQLException {
        ...
        while( !succeeded && tries < MAX_CONNECTION_RETRY ) {
            try {
                tries++;
    ...
    public String convertToAcceptableColumnName(String label, Collection<ColumnFeatures> existingColumns) {
        ...
        if( result.length() > MAX_COLUMN_NAME_LENGTH ) {
            String shorterLabel = result.substring(0,MAX_COLUMN_NAME_LENGTH);
            if( _tableNameForbiddenWords.contains(shorterLabel.toLowerCase()) ) {
                shorterLabel = shorterLabel.substring(0,MAX_COLUMN_NAME_LENGTH-2)+"_c"; //this could only happen if a reserved word is 30 characters long!
            }
            logger.trace("Setting database column name to short version: %s",shorterLabel);
            if( containsColumnAlready(shorterLabel, existingColumns) ) shorterLabel = shorterLabel.substring(0,MAX_COLUMN_NAME_LENGTH-2) + "_2";
            return shorterLabel;
        }
    ...
    public boolean isValidDatabaseTableName( String tableName ) throws InvalidConfigurationException {
        ...
        if( tableName.length() > MAX_TABLE_NAME_LENGTH ) throw new InvalidConfigurationException(String.format("Database table name longer than max 30 characters, by like %d characters. change this name: %s",(tableName.length()-MAX_TABLE_NAME_LENGTH),tableName));
