...
public class Database {
    ...
    String connectionString, user, password, vendorName;
    Table defaultMetricTable, controlTable, defaulEventTable;
    Map<String,Table> tablesMap;

    public Database( String connectionString, String user, String password, String metricTable, String controlTable, String eventTable, Long firstRunHistoricNumberOfHours) throws InvalidConfigurationException {
    ...
    private AnalyticTable getAnalyticTable( Result result ) {
        if( !this.tablesMap.containsKey(result.targetTable) ) {
            AnalyticTable analyticTable = new AnalyticTable( result, this);
            this.tablesMap.put(result.targetTable, analyticTable);
        }
        return (AnalyticTable) this.tablesMap.get(result.targetTable);
    }
    ...
    public void importAnalyticData(Result[] results) {
        ...
        for( Result result : results ) {
            ...
            long startTimeTransaction = Utility.now();
            AnalyticTable table = getAnalyticTable(result);
